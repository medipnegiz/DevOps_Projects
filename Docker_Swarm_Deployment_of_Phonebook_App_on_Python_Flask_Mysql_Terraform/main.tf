...
locals {
  github-repo     = "https://github.com/medipnegiz/phonebook-swarm.git"
  github-file-url = "https://raw.githubusercontent.com/medipnegiz/phonebook-swarm/main/"
}
data "template_file" "leader-master" {
  template = <<EOF
    ...
    yum install git -y
    docker build --force-rm -t "${aws_ecr_repository.ecr-repo.repository_url}:latest" ${local.github-repo}#main     # "--force-rm" sadece son docker layer kalsin, digerlerini sil
    docker push "${aws_ecr_repository.ecr-repo.repository_url}:latest"
    mkdir -p /home/ec2-user/phonebook   # "-p" klasör yoksa olusturmasi icin
